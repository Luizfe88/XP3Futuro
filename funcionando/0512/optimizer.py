...
EMA_SLOW = range(17, 30, 3)
RSI_LEVEL = range(65, 80, 5) # Para compra (Max)
MOMENTUM = [0.2, 0.5, 1.0]
...
def simulate_trades(df, params, side='BUY'):
    ...
    # Nota: Loopar trades é lento, mas necessário para lógica de TP/SL precisa sem lookahead total.
    # Para otimização massiva, converte-se para numpy arrays.
    ...
    for idx in idx_sinais:
        ...
        if side == 'BUY':
            ...
            # Olha o futuro janela de 48 barras
...
def otimizar():
    ...
    logger.info(f"Otimizando COMPRA ({len(list(itertools.product(EMA_FAST, EMA_SLOW, RSI_LEVEL, MOMENTUM)))} combs)...")
    ...
    # Execução Serial para evitar complexidade com ProcessPool no Windows (ou usar if __name__)
    # Para produção real, usar Multiprocessing
    for i, p in enumerate(combs):
        if i % 10 == 0: print(f"Processando {i}/{len(combs)}...", end='\r')
        score, _ = worker_compra((p, dados))
        if score > best_score:
            best_score = score
            best_params = p
            
    print(f"\nMelhor COMPRA: Score {best_score:.2f} | Params: {best_params}")
    
    # Salvar
    res = {
        "score": best_score,
        "ema_fast": best_params[0], "ema_slow": best_params[1],
        "rsi_max": best_params[2], "momentum_min": best_params[3],
