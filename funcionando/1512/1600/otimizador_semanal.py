...
def compute_basic_metrics(equity_curve: List[float]) -> Dict[str, Any]:
    ...
    # Otimização: Não precisamos do loop se usarmos pandas/numpy para returns e std
    # return_series = pd.Series(equity_curve).pct_change().dropna()
    # sharpe = (return_series.mean() / return_series.std()) * (252 ** 0.5) if not return_series.empty and return_series.std() > 0 else 0.0
...
# =========================================================================
# FUNÇÃO OTIMIZADA COM VETORIZAÇÃO (Substitui o loop Python lento)
# =========================================================================
def backtest_params_on_df(sym: str, params: dict, df):
    # 1. Tenta usar o backtest otimizado do usuário (base.test_params_on_data)
    try:
        if base and hasattr(base, "test_params_on_data"):
            # SUGESTÃO: Certifique-se de que a sua base.test_params_on_data é 100% vetorizada!
            return base.test_params_on_data(sym, params, df)
    except Exception:
        logger.warning(f"Base backtest falhou para {sym}. Usando fallback otimizado.")
        pass

    # 2. Fallback Otimizado e Vetorizado: Simulação de Cruzamento de EMA
    try:
        ...
        # Cálculo de EMAs (Vetorizado)
        ema_s = close.ewm(span=short, adjust=False).mean()
        ema_l = close.ewm(span=long, adjust=False).mean()

        # Geração de Posição (Vetorizado)
        # O sinal é 1 se EMA curta > EMA longa, 0 caso contrário
        signal = (ema_s > ema_l).astype(int)

        # Posição efetiva: Usa o sinal deslocado, simulando que abrimos a posição
        # no candle seguinte após o sinal.
        pos = signal.shift(1).fillna(0) 

        # Cálculo dos Retornos Diários
        daily_returns = close.pct_change().fillna(0)

        # Retornos da Estratégia: Retorno * Posição (ganhamos retorno apenas quando posicionados)
        strategy_returns = daily_returns * pos
        
        # Curva de Capital (Equity): Calcula o produto acumulado dos retornos (composição)
        equity_curve = (1 + strategy_returns).cumprod()
        
        # A lista de equity para métricas deve começar em 1.0 (capital inicial)
        equity_list = [1.0] + equity_curve.iloc[1:].tolist()

        metrics = compute_basic_metrics(equity_list)
        return {"metrics": metrics, "equity_curve": equity_list}

    except Exception as e:
        logger.warning(f"Fallback backtest falhou com erro: {e}. Retornando resultado vazio.")
        return {"metrics": {}, "equity_curve": [1.0]}
# =========================================================================

def optimize_window(sym: str, df_train, maxevals: int):
    # SUGESTÃO: Se optimize_on_data/optimize_symbol_robust for lento,
    # considere usar um Otimizador Bayesiano (como Optuna) no seu módulo 'base'.
    # Algoritmos Bayesianos podem encontrar soluções ótimas com muito menos avaliações (<300).
    try:
...
def worker_wfo(sym: str, bars: int, maxevals: int, wfo_windows: int, train_period: int, test_period: int) -> Dict[str, Any]:
    ...
    try:
        ...
        def score_win(w):
            ...
            # SUGESTÃO: Ajuste esta métrica para ser mais robusta.
            # Ex: (Sharpe * 0.7) + (Profit Factor * 0.2) + (Total Return * 0.1)
