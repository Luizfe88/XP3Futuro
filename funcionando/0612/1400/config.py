...
OPTIMIZER_HISTORY_FILE = "data/optimizer_history.json" # Para Walk-Forward

# ==================== HORÁRIO DE OPERAÇÃO ====================
TIMEFRAME_MT5 = mt5.TIMEFRAME_M5 # Timeframe principal de análise
START_TIME = datetime.time(10, 15) 
END_TIME = datetime.time(16, 45)   
CHECK_INTERVAL_SLOW = 15  # Loop principal

# ==================== PARÂMETROS ADAPTATIVOS & REGIMES V2 (PONTO 1) ====================
IBOV_SYMBOL = "WINJ25" # Exemplo de índice futuro (Proxy do IBOV)
IBOV_MM_PERIOD = 200
IBOV_ADX_SIDEWAYS = 20.0 # Threshold para SIDEWAYS

# Novos arquivos de parâmetros
...
# Parâmetros Default (Fallback)
DEFAULT_PARAMS = {
    "ema_fast": 14, "ema_slow": 30, "rsi_level": 70, "momentum_min": 0.0, 
    "adx_min": 20, "sl_atr_mult": 2.0, "tp_mult": 1.5, "side": "NEUTRO"
}

# ==================== GESTÃO DE RISCO DE PORTFÓLIO ====================

# Vol Targeting Dinâmico (PONTO 2)
USE_VOL_TARGETING = True
TARGET_VOL_ANNUAL = 0.20    # Alvo de volatilidade anualizada (20%)
MAX_LEVERAGE = 3.0          # Alavancagem nocional máxima
MIN_LEVERAGE_FLOOR = 0.3    # Fator mínimo de multiplicação do risco

# Portfolio Heat & Diversificação (PONTO 3)
MAX_POSITIONS = 5           # Máximo de posições simultâneas
MAX_POSITIONS_PER_SECTOR = 2 # Máximo de posições por setor
MAX_CORRELATION_BLOCK = 0.75 # Bloqueia entrada se correlação rolling 30d > 0.75

# Risk Overlays (VIX & Beta) (PONTO 6)
VIX_CRISIS_THRESHOLD = 55.0 # Se Volatilidade Anualizada do WIN > 55%
CRISIS_RISK_REDUCTION = 0.15 # Risco de 0.15% em CRISIS/VIX ALTO
MAX_EXPOSURE_PER_ASSET_PCT = 10.0 # Máximo 10% do Equity em uma única ação

# Lista Fixa de Ativos de Alto Beta (Ações com Beta > 2.0 em crises)
HIGH_BETA_BLOCK_LIST = ["MGLU3", "AZUL4", "GOLL4", "CVCB3", "COGN3", "LREN3", "AMER3"] 
# Adicione outras se necessário, esta é uma proxy de alto risco cíclico/varejo.

# Saídas Inteligentes (Partial Profits & Time Stop) (PONTO 4)
PARTIAL_PROFIT_1_R = 1.0    # Lucro em R para a 1ª saída parcial
PARTIAL_PROFIT_1_PCT = 0.3  # % volume a ser vendido na 1ª saída
PARTIAL_PROFIT_2_R = 1.8    # Lucro em R para a 2ª saída parcial
PARTIAL_PROFIT_2_PCT = 0.4  # % volume a ser vendido na 2ª saída (deixando 30%)
TIME_STOP_HOURS = 8         # Tempo máximo em horas
TIME_STOP_PROFIT_MIN_R = 0.5 # Lucro mínimo em R para ativar time stop

# ==================== EXECUÇÃO & SEGURANÇA ====================

# Execução Iceberg V2 (PONTO 5)
USE_ICEBERG_EXECUTION = True
ICEBERG_SPLIT = 5           # Ordem fatiada em N pedaços
ICEBERG_MIN_DELAY = 1.0     # Delay mínimo entre legs (segundos)
ICEBERG_MAX_DELAY = 3.0     # Delay máximo

# Segurança & Robustez (PONTO 8)
HEARTBEAT_INTERVAL_SEC = 120 # Se sem tick há 120s, fechar tudo
MAX_LOG_SILENCE_MIN = 5     # Tempo máximo sem log (para watchdog externo, não implementado aqui)
PANIC_DROP_PCT = -2.0       # Se equity cai 2% em
PANIC_DROP_WINDOW_MIN = 10  # 10 minutos

# ==================== DICIONÁRIO DE SETORES (PONTO 3) ====================
# Utilizado para o filtro de diversificação (MAX_POSITIONS_PER_SECTOR)
# Mantenha o seu dicionário SETORES já existente.
CANDIDATOS_BASE = [
