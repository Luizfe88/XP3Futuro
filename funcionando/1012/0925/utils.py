...
def prepare_data_for_scan(symbol, params, lookback_days=300):
    rates = data_cache.get_data(symbol, config.TIMEFRAME_MT5, 1000)
    ...
    # === CÁLCULO VWAP (FILTRO INSTITUCIONAL) ===
    # VWAP Intra-diária (Ancorada no início dos dados carregados ou usando pandas_ta rolling)
    # Aqui usaremos a VWAP "rolling" do pandas_ta como proxy de tendência curto prazo
    try:
        df.ta.vwap(append=True) # Cria coluna VWAP_D (Daily Anchor) geralmente
        # Se pandas_ta não criar VWAP_D, criamos uma manual simples hlc3 * vol
        if 'VWAP_D' not in df.columns:
             tp = (df['High'] + df['Low'] + df['Close']) / 3
             df['VWAP_D'] = (tp * df['Volume']).cumsum() / df['Volume'].cumsum()
    except:
         pass # Segue sem VWAP se der erro
...
def check_trade_signal(df, params, side):
    ...
    # Validações básicas de colunas
    if f"EMA_{params['ema_fast']}" not in df.columns: return False, {}

    ema_f = last[f"EMA_{params['ema_fast']}"]
    ema_s = last[f"EMA_{params['ema_slow']}"]
    ...
    if vwap == 0: vwap = price # Ignora se não calculado
    
    # Lógica Institucional:
    # COMPRA: Preço deve estar ACIMA da VWAP (Institucionais defendendo preço médio)
    # VENDA: Preço deve estar ABAIXO da VWAP
    ...
    if side == "COMPRA":
        ...
        cond_rsi = rsi >= params['rsi_level']
        cond_adx = adx >= params['adx_min']
        cond_mom = mom >= params['momentum_min_pct']
        
        # Filtro VWAP Compra
        if price < vwap:
            vwap_condition = False
            vwap_status = "ABAIXO (Perigoso)"
        else:
            vwap_status = "ACIMA (OK)"
        ...
        if price > vwap:
            vwap_condition = False
            vwap_status = "ACIMA (Perigoso)"
        else:
            vwap_status = "ABAIXO (OK)"

    # Só aprova se VWAP permitir (você pode comentar essa linha se quiser ser menos rigoroso no inicio)
    aprovado = cond_ema and cond_rsi and cond_adx and cond_mom and vwap_condition
    ...
    detalhes = {
        ...
        "ADX": f"{adx:.1f}",
        "MOM": f"{mom:.2f}%",
        "VWAP": vwap_status, # Adicionado ao log
        "STATUS": f"{Fore.GREEN if aprovado else Fore.RED}{'APROVADO' if aprovado else 'REPROVADO'}{Style.RESET_ALL}",
...
# === CÓDIGO DE VENCIMENTO AUTOMÁTICO ===
def get_current_future_symbol(base_symbol, vencimentos_map):
    """
    Calcula e retorna o contrato futuro vigente (ex: WING26).
    Usa o mês e ano atual para determinar o próximo vencimento.
    """
    now = datetime.now()
    
    # Mapeia o mês atual para o próximo mês de vencimento par
    proximo_vencimento_mes = 0
    
    # Itera sobre os meses pares
    for mes_vencimento in sorted(vencimentos_map.keys()):
        if now.month <= mes_vencimento:
            proximo_vencimento_mes = mes_vencimento
            break
            
    # Se o mês atual for Dezembro (12), o próximo vencimento é Fevereiro do ano seguinte
    if proximo_vencimento_mes == 0:
        proximo_vencimento_mes = 2 # Fevereiro
        ano = now.year + 1
    else:
        ano = now.year
        
    # Pega a letra do mapa
    letra_vencimento = vencimentos_map[proximo_vencimento_mes]
    
    # Formato do ano (ex: 26)
    ano_str = str(ano)[-2:]
    
    # Junta tudo (ex: WING26)
    current_symbol = f"{base_symbol}{letra_vencimento}{ano_str}"
    
    logging.info(f"{AZUL}Contrato Futuro Vigente (WIN/IBOV): {current_symbol}{RESET}")
    return current_symbol

def get_market_regime():
    """
    Define o regime de mercado baseado na Média Móvel de 200 do IBOV (D1).
    Retorna: Regime (str), Preço Atual, MM200, Volatilidade (VIX fake ou ATR)
    """
    ibov_symbol_name = get_current_future_symbol(config.IBOV_BASE_SYMBOL, config.VENCIMENTOS_MAP)
    symbol = ibov_symbol_name # Certifique-se que é "WINZ25" ou o atual, ou "IBOV" (Indice)
    
    # Tenta pegar dados diários (D1)
    rates = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_D1, 0, 202)
    
    if rates is None or len(rates) < 200:
        # Fallback se falhar o IBOV futuro, tenta o índice spot se disponível
        logging.warning("Falha ao ler dados do IBOV. Usando regime NEUTRO de segurança.")
        return "SIDEWAYS", 0.0, 0.0, 0.0

    df = pd.DataFrame(rates)
    current_price = df['close'].iloc[-1]
    
    # Calcula MM200 Simples
    sma200 = df['close'].rolling(window=200).mean().iloc[-1]
    
    # Define Regime
    # Se preço > MM200 + 1% de filtro = BULL
    if current_price > (sma200 * 1.01):
        regime = "BULL"
        if current_price > (sma200 * 1.05): # 5% acima da média = Forte
            regime = "STRONG_BULL"
            
    # Se preço < MM200 - 1% de filtro = BEAR
    elif current_price < (sma200 * 0.99):
        regime = "BEAR"
        if current_price < (sma200 * 0.90): # 10% abaixo da média = Crise
            regime = "CRISIS"
            
    else:
        regime = "SIDEWAYS" # Entre a média e os filtros

    logging.info(f"{Fore.CYAN}REGIME DE MERCADO DETECTADO: {regime} (IBOV: {current_price} | MM200: {sma200:.0f}){Style.RESET_ALL}")
    
    # Retorna valores reais
    return regime, current_price, sma200, 0.0
...
def aplicar_trailing_stop_adaptativo(positions, lookback_bars=25):
    """
    Trailing Stop Inteligente + Breakeven Automático
    - Ativa Breakeven após +1.0R
    - Ativa Trailing após +1.5R
    - Nunca retrocede o SL (ratchet)
    - Totalmente adaptativo ao ATR do ativo
    """
    if not positions:
        return
    ...
    for pos in positions:
        ...
        sl_atual = pos.sl if pos.sl > 0.01 else 0.0
        tp = pos.tp
        profit = pos.profit
        volume = pos.volume

        # === 1. ATR atual do ativo ===
        try:
            rates = mt5.copy_rates_from_pos(symbol, config.TIMEFRAME_MT5, 0, lookback_bars + 14)
            ...
            if pd.isna(atr) or atr <= 0:
                atr = preco_at * 0.008  # fallback seguro
        except Exception as e:
            atr = preco_at * 0.008

        # === 2. Risco inicial (distância do SL original ou 2x ATR se não tiver SL) ===
        risco_inicial = abs(preco_abertura - sl_atual) if sl_atual > 0.01 else (atr * 2.0)
        # Garante um risco mínimo razoável
        risco_inicial = max(risco_inicial, atr * 1.5)

        # === 3. Lucro em múltiplos de R ===
        if risco_inicial == 0:
            continue
        lucro_em_r = profit / (volume * risco_inicial)

        # === 4. BREAKEVEN AUTOMÁTICO (+1.0R) ===
        if lucro_em_r >= 1.0 and sl_atual < preco_abertura * (1.001 if tipo == mt5.POSITION_TYPE_BUY else 0.999):
            novo_sl = round(preco_abertura * 1.001, 2) if tipo == mt5.POSITION_TYPE_BUY else round(preco_abertura * 0.999, 2)

            if (tipo == mt5.POSITION_TYPE_BUY and novo_sl > sl_atual) or \
               (tipo == mt5.POSITION_TYPE_SELL and novo_sl < sl_atual):
                _enviar_modificacao_sl(ticket, symbol, novo_sl, tp)
                logger.info(f"{VERDE}BREAKEVEN ATIVADO → {symbol} | SL movido para {novo_sl:.2f} (+1R){RESET}")
            continue

        # === 5. TRAILING STOP ADAPTATIVO (+1.5R ou mais) ===
        if lucro_em_r < 1.5:
            continue  # ainda não ativou trailing

        mult_trailing = 2.2  # padrão
        if atr > preco_at * 0.015:      # alta volatilidade → afasta mais
            mult_trailing = 2.8
        elif atr < preco_at * 0.006:    # baixa vol → aperta
            mult_trailing = 1.7

        if tipo == mt5.POSITION_TYPE_BUY:
            novo_sl = round(preco_at - (atr * mult_trailing), 2)
            # Ratchet: só melhora
            if novo_sl > sl_atual:
                _enviar_modificacao_sl(ticket, symbol, novo_sl, tp)
                logger.info(f"{VERDE}TRAILING → {symbol} | SL: {sl_atual:.2f} → {novo_sl:.2f} (+{lucro_em_r:.1f}R){RESET}")

        else:  # VENDA
            novo_sl = round(preco_at + (atr * mult_trailing), 2)
            if novo_sl < sl_atual or sl_atual == 0:
                _enviar_modificacao_sl(ticket, symbol, novo_sl, tp)
                logger.info(f"{VERDE}TRAILING (VENDA) → {symbol} | SL: {sl_atual:.2f} → {novo_sl:.2f} (+{lucro_em_r:.1f}R){RESET}")
...
def aplicar_trailing_stop_adaptativo(positions):
    ...
    for pos in positions:
        ...
        # ATR
        rates = mt5.copy_rates_from_pos(symbol, config.TIMEFRAME_MT5, 0, 40)
        ...
        # Risco inicial
        risco_inicial = abs(open_price - sl_atual) if sl_atual > 0.01 else atr * 2.0
        risco_inicial = max(risco_inicial, atr * 1.5)
        lucro_r = profit / (volume * risco_inicial) if risco_inicial > 0 else 0

        # BREAKEVEN +1R
        if lucro_r >= 1.0 and ((tipo == mt5.POSITION_TYPE_BUY and sl_atual < open_price * 1.001) or
                               (tipo == mt5.POSITION_TYPE_SELL and sl_atual > open_price * 0.999)):
            novo_sl = round(open_price * 1.001, 2) if tipo == mt5.POSITION_TYPE_BUY else round(open_price * 0.999, 2)
            _enviar_sltp(ticket, symbol, novo_sl, tp)
            logger.info(f"{VERDE}BREAKEVEN → {symbol}{RESET}")
            continue

        # TRAILING após +1.5R
        if lucro_r >= 1.5:
            mult = 2.2
            if atr > current_price * 0.015: mult = 2.8
            if atr < current_price * 0.006: mult = 1.7

            if tipo == mt5.POSITION_TYPE_BUY:
                novo_sl = round(current_price - atr * mult, 2)
                if novo_sl > sl_atual:
                    _enviar_sltp(ticket, symbol, novo_sl, tp)
                    logger.info(f"{VERDE}TRAILING → {symbol} SL {sl_atual:.2f} → {novo_sl:.2f}{RESET}")
            else:
                novo_sl = round(current_price + atr * mult, 2)
                if novo_sl < sl_atual or sl_atual == 0:
                    _enviar_sltp(ticket, symbol, novo_sl, tp)
                    logger.info(f"{VERDE}TRAILING VENDA → {symbol}{RESET}")
