...
✅ Validação robusta
✅ FIX: "No trials are completed yet"
"""
...
def compute_metrics(equity_curve):
    ...
    if not equity_curve or len(equity_curve) < 50:
...
def objective(trial, symbol, df):
    """Função objetivo do Optuna com timeout interno"""
    try:
        params = {
            "ema_short": trial.suggest_int("ema_short", 8, 25),
            "ema_long": trial.suggest_int("ema_long", 40, 200),
            "rsi_low": trial.suggest_int("rsi_low", 25, 35),
            ...
            "sl_atr_multiplier": trial.suggest_float("sl_atr_multiplier", 1.0, 1.8)
        }
        ...
        # Penalty extra para DD
        dd_penalty_extra = max(0, max_dd - 0.2) * 10
        ...
        logger.error(f"{symbol}: Erro no trial: {e}")
        return 999.0  # Penalidade alta para trials com erro


# =========================================================
# FUNÇÃO PRINCIPAL DE OTIMIZAÇÃO
# =========================================================
def optimize_with_optuna(sym: str, df_train, n_trials: int = 60, timeout: int = 180):
    """
    Otimização RÁPIDA com Optuna
    ✅ FIX: Verifica se há trials completos ANTES de acessar best_trial
    """
    ...
    try:
        ...
        logger.error(f"{sym}: ❌ Optuna optimize() falhou: {e}")
        raise

    # ✅ FIX CRÍTICO: Verifica se há trials completos
    completed_trials = [t for t in study.trials if t.state == optuna.trial.TrialState.COMPLETE]
    
    if not completed_trials:
        logger.error(f"{sym}: ❌ NENHUM TRIAL COMPLETADO em {timeout}s!")
        logger.error(f"{sym}: Total de trials: {len(study.trials)}")
        logger.error(f"{sym}: Estados: {[t.state for t in study.trials[:5]]}")
        raise ValueError(f"{sym}: Timeout - nenhum trial completou em {timeout}s")

    # Agora é seguro acessar best_trial
    best_params = study.best_params
    best_score = -study.best_value
    attrs = study.best_trial.user_attrs

    logger.info(
        f"{sym}: ✅ {len(completed_trials)}/{len(study.trials)} trials completos | "
