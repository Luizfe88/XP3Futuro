...
def can_add_pyramid_position(symbol, current_pos):
    # Regra de Ouro: Só escalamos se o Stop Loss da 1ª posição já estiver no Breakeven 
    # ou se o lucro aberto cobrir o risco da nova entrada.
    if current_pos.profit > 0 and current_pos.price_current > current_pos.price_open:
        return True
    return False
...
def is_market_liquid(df):
    # Compara o volume atual com a média das últimas 20 barras
    avg_volume = df['tick_volume'].rolling(20).mean().iloc[-1]
    current_volume = df['tick_volume'].iloc[-1]
    return current_volume > (avg_volume * 0.8) # Evita "vazios" de liquidez
...
def executar_ordem_manual(symbol, side="BUY", volume=100):
    """
    Comando rápido para compra/venda manual via código.
    side: "BUY" ou "SELL"
    volume: Quantidade de ações (ex: 100)
    """
    import MetaTrader5 as mt5
    ...
    # Monta a requisição usando o padrão de preenchimento da B3 (IOC/RETURN)
    request = {
        "action": mt5.TRADE_ACTION_DEAL,
        "symbol": symbol,
        "volume": float(volume),
        "type": order_type,
        "price": float(price),
        "magic": 777777, # Identificador de ordem manual
        "comment": "EXECUCAO MANUAL XP3",
    ...
    if result.retcode != mt5.TRADE_RETCODE_DONE:
        print(f"❌ Falha na ordem manual: {result.retcode}")
    else:
        print(f"✅ Ordem de {side} em {symbol} ({volume} ações) executada com sucesso!")
...
def main():
    ...
    #executar_ordem_manual("PETR4", side="BUY", volume=100)
    #executar_ordem_manual("PETR4", side="SELL", volume=200)
