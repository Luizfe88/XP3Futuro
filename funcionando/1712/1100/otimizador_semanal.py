...
def compute_basic_metrics(equity_curve: List[float]) -> Dict[str, Any]:
    ...
    years = len(equity_curve) / (252 * 16)  # M15 → ~4032 barras por ano
...
def backtest_params_on_df(symbol, params, df) -> Dict[str, Any]:
    ...
    # EMAs
    ema_short = pd.Series(close).ewm(span=params.get("ema_short", 9), adjust=False).mean().values
    ema_long = pd.Series(close).ewm(span=params.get("ema_long", 21), adjust=False).mean().values
    ...
    equity_curve = [cash]
    trades = 0
    slippage = 0.0012  # 0.12% round-trip realista B3

    start_idx = max(params.get("ema_long", 50), 50)
    ...
    for i in range(start_idx, len(close)):
        ...
        if position == 0:
            if buy_signal:
                ...
                stop_price = entry_price - curr_atr * config.SL_ATR_MULTIPLIER
                risk_per_share = entry_price - stop_price
                if risk_per_share <= 0:
                    continue
                # Risco fixo 1%
                volume = (cash * 0.01) / risk_per_share
                trades += 1
            elif sell_signal:
                position = -1
                entry_price = price * (1 - slippage / 2)
...
if __name__ == "__main__":
    ...
    for sym, res in elite_results.items():
        ...
        if (mc["win_rate"] >= 0.65 and 
            mc["calmar_avg"] >= 1.2 and 
            mc["calmar_5th"] >= 0.5 and      # só aceita positivo no pior 5%
            mc["max_dd_95"] <= 0.30):         # max 30% drawdown no pior 5%
