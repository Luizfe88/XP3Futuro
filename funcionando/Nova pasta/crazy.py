...
def execute_trade(symbol, order_type, lot_size, execution_type="market", limit_price=None):
    ...
    # === CONFIGURAÇÃO PARA AÇÕES DA B3 (em centavos) ===
    if symbol in ["AZUL4", "PETR4", "VALE3", "ITUB4", "BBDC4", "PRIO3", "WEGE3", "ABEV3"]:
        if symbol == "AZUL4":
            sl_points = 0.15   # 15 centavos de SL
            tp_points = 0.30   # 30 centavos de TP
        elif symbol == "PETR4":
            sl_points = 0.30   # 30 centavos SL
            tp_points = 0.70   # 70 centavos TP
        else:
            sl_points = 0.20
            tp_points = 0.50
    else:
...
symbol_dropdown['values'] = (
    "AZUL4", "PETR4", "VALE3", "ITUB4", "BBDC4", "PRIO3", "WEGE3", "ABEV3",
    "GGBR4", "CSNA3", "USIM5", "XAUUSDm", "BTCUSDm", "EURUSDm", "USDJPYm"
)
