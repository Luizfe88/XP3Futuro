...
def generate_scanner_top10_elite(scan_results, top_n=10):
    """
    Scanner TOP 10 ELITE – Mostra RSI, EMA, Volume, Momentum e MOTIVO exato de não entrar.
    TESTADO: Funciona com top_n como parâmetro.
    """
    if not scan_results:
        return f"\n{AMARELO}SCANNER VAZIO – Nenhum ativo analisado{RESET}\n"

    # Filtra só dados válidos
    validos = [r for r in scan_results if isinstance(r, dict) and r.get('indicators') and 'symbol' in r]
    if not validos:
        return f"\n{AMARELO}NENHUM ATIVO COM DADOS VÁLIDOS{RESET}\n"

    # Score: prioriza setups extremos (RSI baixo/alto + volume + momentum)
    def calcular_score(r):
        rsi = r['indicators'].get('RSI', 50)
        vol_mult = r['indicators'].get('VOL_CURRENT', 0) / max(r['indicators'].get('VOL_AVG', 1), 1)
        mom = abs(r.get('momentum_pct', 0))  # Assume que você calcula momentum em analyze_symbol_for_trade
        dist_ema = abs(r['price_current'] - r['indicators'].get('EMA', r['price_current']))
        score = (min(rsi, 100 - rsi) * 2) + (vol_mult * 8) + (mom * 6) - (dist_ema * 2)
        return score

    validos.sort(key=calcular_score, reverse=True)

    report = f"\n{ROXO}=== SCANNER ELITE 2026 – TOP {min(top_n, len(validos))} OPORTUNIDADES ==={RESET}\n"
    report += "RANK │ SÍMBOLO   │ PREÇO    │ RSI   │ EMA200  │ VOLxMÉD │ MOM%   │ STATUS        │ MOTIVO SE REPROVADO\n"
    report += "─────┼───────────┼──────────┼───────┼─────────┼─────────┼────────┼───────────────┼─────────────────────────────\n"

    for i, data in enumerate(validos[:top_n], 1):
        s = data['symbol']
        p = data['price_current']
        ind = data['indicators']
        rsi = ind.get('RSI', 0.0)
        ema = ind.get('EMA', 0.0)  # EMA200
        vol_atual = ind.get('VOL_CURRENT', 0)
        vol_med = ind.get('VOL_AVG', 1)
        vol_mult = vol_atual / vol_med if vol_med > 0 else 0
        mom = data.get('momentum_pct', 0.0)  # % de mudança recente (adicione em analyze_symbol_for_trade se não tiver)

        # Cores
        cor_rsi = VERDE if rsi < 35 or rsi > 65 else AMARELO
        cor_vol = VERDE if vol_mult >= 2.0 else (AMARELO if vol_mult >= 1.3 else "")
        cor_mom = VERDE if abs(mom) >= 2.0 else AMARELO

        # Status e Motivo
        if data.get('can_buy') and data.get('can_sell'):
            status = f"{AMARELO}CONFLITO{RESET}"
            motivo = "Sinal duplo (raro)"
        elif data.get('can_buy'):
            status = f"{VERDE}COMPRA OK{RESET}"
            motivo = "— (Setup completo)"
        elif data.get('can_sell'):
            status = f"{VERMELHO}VENDA OK{RESET}"
            motivo = "— (Setup completo)"
        else:
            status = f"{AZUL}AGUARDANDO{RESET}"
            motivos = []
            if 35 <= rsi <= 65:
                motivos.append("RSI neutro")
            if vol_mult < 1.5:
                motivos.append(f"Vol {vol_mult:.1f}x")
            if abs(mom) < 1.5:
                motivos.append("Mom baixo")
            if abs(p - ema) / p > 0.02:  # Distante da EMA
                motivos.append("Longe EMA")
            motivo = " | ".join(motivos) if motivos else "Filtro custom"

        report += (
            f"{i:>3}  │ "
            f"{s:<9} │ "
            f"{p:>8.2f} │ "
            f"{cor_rsi}{rsi:>5.1f}{RESET} │ "
            f"{ema:>7.2f} │ "
            f"{cor_vol}{vol_mult:>5.1f}x{RESET} │ "
            f"{cor_mom}{mom:+6.1f}%{RESET} │ "
            f"{status:<13} │ {motivo:<25}\n"
        )

    report += f"\n{ROXO}Total: {len(validos)} ativos | {datetime.datetime.now().strftime('%H:%M:%S')}{RESET}\n"
    return report

# --- Em utils.py: Adicione no final do arquivo ---

# --- Em utils.py: SUBSTITUA A FUNÇÃO is_market_open POR ESTA CORRIGIDA ---
