...
import config
import utils
from utils import logger, Fore, Style, VERDE, AMARELO, VERMELHO, AZUL, ROXO, RESET, generate_positions_report, analyze_symbol_for_trade, generate_scanner_top10_elite
...
def execute_iceberg_order(symbol, side, lote_calc, price_current, sl_price_simulado, tp_price_simulado):
    ...
    # === PASSO 1: Tentativa Atômica (melhor caso) ===
    request = {
    ...
    if result.retcode == mt5.TRADE_RETCODE_DONE:
        logger.info(f"{VERDE}ABERTURA ATÔMICA COM SL/TP → {symbol} {side_name} {volume:,} lotes{RESET}")
        return result.order  # ticket

    # === PASSO 2: Se falhou → Abre SEM SL/TP (urgente) e depois corrige ===
    logger.warning(f"{AMARELO}Atômica falhou ({result.comment}). Abrindo sem SL e forçando depois...{RESET}")
    ...
    result = mt5.order_send(request_no_st)
    if result.retcode != mt5.TRADE_RETCODE_DONE:
        logger.critical(f"{VERMELHO}FALHA TOTAL NA ABERTURA DE {symbol}: {result.comment}{RESET}")
        return None

    ticket = result.order
    logger.warning(f"{AMARELO}Posição aberta SEM proteção (Ticket {ticket}). Iniciando modo GUARDIÃO...{RESET}")

    # === PASSO 3: Modo GUARDIÃO – Persistência até colocar SL/TP ===
    for tentativa in range(1, max_retries + 1):
        time.sleep(0.6 + tentativa * 0.4)  # Backoff exponencial leve
        ...
        logger.warning(f"Tentativa {tentativa}/{max_retries} falhou: {res.comment}")
...
def ciclo_principal():
    ...
    if positions and not simulacao:  # não ativa em simulação
    ...
    print(utils.generate_scanner_top10_elite(scanner_results, top_n=10))
            ...
            if not FORCAR_MODO_TESTE_24H:
                if not (config.START_TIME <= current_time <= config.END_TIME):
                    ...
                    if not utils.is_market_open("WINZ25"):  # ou PETR4, VALE3
                        ...
                        ciclo_principal(simulacao=True)  # <--- nova flag
                        time.sleep(10)
                        continue
