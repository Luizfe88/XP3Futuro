...
# --- Risk and Macro timeframe settings (from config) ---
RISK_PER_TRADE_PCT = getattr(config, 'RISK_PER_TRADE_PCT', 0.01) if config else 0.01
MACRO_TF_STR = getattr(config, 'MACRO_TIMEFRAME', 'H1') if config else 'H1'
MACRO_EMA_LONG = int(getattr(config, 'MACRO_EMA_LONG', 200) if config else 200)
MACRO_TIMEFRAME_MT5 = TF_MAP.get(MACRO_TF_STR, mt5.TIMEFRAME_H1)
MAX_TOTAL_DRAWDOWN_PCT = getattr(config, 'MAX_TOTAL_DRAWDOWN_PCT', 0.03) if config else 0.03
RESET_THRESHOLD = float(getattr(config, 'CB_RESET_THRESHOLD', 0.7)) if config else 0.7
# peak equity tracking for total drawdown checks
peak_equity = None


def should_execute_side(symbol: str, side: str) -> bool:
    """
    Confirma se uma ordem de given side ('BUY'/'SELL') deve ser enviada com base na tendência macro.
    BUY -> só se close_macro > macro_ema
    SELL -> só se close_macro < macro_ema
    Se não houver dados macro, retornamos True (fallback permissivo).
    """
    try:
        if not utils or not hasattr(utils, 'safe_copy_rates'):
            return True
        macro_df = utils.safe_copy_rates(symbol, MACRO_TIMEFRAME_MT5, 300)
        if macro_df is None or macro_df.empty:
            return True
        close_macro = macro_df['close'].astype(float)
        macro_ema = float(close_macro.ewm(span=MACRO_EMA_LONG, adjust=False).mean().iloc[-1])
        last_close_macro = float(close_macro.iloc[-1])
        if side.upper() == 'BUY':
            return last_close_macro > macro_ema
        else:
            return last_close_macro < macro_ema
    except Exception:
        return True
...
def render_panel(equity: float, portfolio_indicators: Dict[str, Dict[str,Any]], positions: List[Dict[str,Any]]):
    ...
    print(C_BOLD + C_MAGENTA + "FAST BOT PANEL – VERSÃO DEFINITIVA" + C_RESET)
    print(f"{C_YELLOW}Horário: {now_str}{C_RESET}    Fast: {FAST_INTERVAL_SEC}s    Slow: {SLOW_INTERVAL_SEC}s")
    print(f"Equity: {C_BOLD}{C_YELLOW}{equity:,.2f} BRL{C_RESET}\n")

    # POSIÇÕES ABERTAS
    print(C_BOLD + C_CYAN + "POSIÇÕES ABERTAS" + C_RESET)
    if not positions:
        print("  (sem posições)")
    else:
        for p in positions:
            prof = p.get("profit", 0.0)
            col = C_GREEN if prof >= 0 else C_RED
            print(f"  {p['symbol']:6} {p['volume']:6.2f} @ {p['price']:8.2f}  SL {p['sl']:.4f}  {col}P&L {prof:8.2f}{C_RESET}")
    print("")
    ...
    for sym, d in portfolio_indicators.items():
        ...
        last_vol  = d.get("last_vol") or 0
        vol_mean  = d.get("vol_mean") or 0

        targets = d.get("targets") or load_optimizer_targets(sym)

        adx_th   = targets.get("adx_threshold", 20)
        rsi_low  = targets.get("rsi_low", 30)
        rsi_high = targets.get("rsi_high", 70)
        mom_min  = targets.get("mom_min", 0.0)

        # O QUE ESTÁ FALTANDO?
        faltas = []
        if not (ema_f and ema_s and ema_f > ema_s):
            faltas.append("EMA")
        if adx < adx_th:
            faltas.append(f"ADX>{int(adx_th)}")
        if rsi is None or not (rsi_low <= rsi <= rsi_high):
            faltas.append("RSI")
        if mom is None or mom < mom_min:
            faltas.append("MOM")
        if not should_execute_side(sym, "BUY"):
            faltas.append("MACRO")

        sinal = C_GREEN + C_BOLD + "PRONTO!" + C_RESET if len(faltas) == 0 else C_RED + "|".join(faltas) + C_RESET

        # Score
        score = 0
        if ema_f and ema_s and ema_f > ema_s:  score += 40
        if adx >= adx_th:                      score += 30
        if rsi and rsi_low <= rsi <= rsi_high: score += 20
        if mom and mom >= mom_min:             score += 10
        if len(faltas) == 0:                   score += 200  # joga pro topo

        # Última barra
        last_bar_time = "??:??"
        try:
            df_tmp = utils.safe_copy_rates(sym, TIMEFRAME_MT5, 2)
            if df_tmp is not None and len(df_tmp) >= 1:
                last_bar_time = df_tmp.index[-1].strftime("%H:%M")
        except:
            pass

        # TUPLA COMPLETA (agora com last_vol e vol_mean)
        rows.append((
            score, sym, price, ema_f, ema_s, rsi, adx, adx_th,
            last_vol, vol_mean, last_bar_time, sinal, targets
        ))
    ...
    # CABEÇALHO
    print(f"{C_BOLD + C_MAGENTA}{'POS':>3} {'SYM':6} {'PREÇO':>10} {'EMA f>s':>18} {'RSI':>7} {'ADX':>6} {'TH':>4} {'VOL L/M':>12} {'ÚLT':>6} {'FALTA PARA COMPRA':>20} {'OPT'}{C_RESET}")
    print("─" * 145)

    for i, (score, sym, price, ema_f, ema_s, rsi, adx, adx_th,
            last_vol, vol_mean, last_time, sinal, targets) in enumerate(rows):
        if i >= 40:
            break
        pos = f"{i+1:02d}º"

        linha = C_GREEN + C_BOLD if "PRONTO!" in sinal else (C_YELLOW if "EMA" not in sinal and "MACRO" not in sinal else "")

        ema_str = f"{ema_f:.2f}>{ema_s:.2f}" if ema_f and ema_s else "N/A"
        rsi_str = f"{rsi:.1f}" if rsi is not None else "N/A"
        adx_str = f"{adx:.1f}"
        vol_str = f"{int(last_vol):,}/{int(vol_mean):,}"

        print(f"{linha}{pos:>3} {sym:6} {price:10.2f} {ema_str:>18} {rsi_str:>7} {adx_str:>6} {int(adx_th):4} {vol_str:>12} {last_time:>6} {sinal:>20} EMA {targets.get('ema_fast',9)}/{targets.get('ema_slow',21)}{C_RESET}")
        ...
        try:
            for sym in symbols:
                ...
                if trade_ok and not has_pos:
                    ...
                    # macro confirmation for BUY/SELL
                    if not should_execute_side(sym, 'BUY'):
                        continue
                    lots = utils.calculate_position_size(sym, sl_price, risk_pct=RISK_PER_TRADE_PCT) if utils and hasattr(utils, 'calculate_position_size') else None
            ...
            logger.exception(f"Fast loop error for {sym}")
