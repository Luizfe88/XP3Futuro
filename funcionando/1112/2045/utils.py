# utils.py — utilitários do bot (versão atualizada com fallback para copy_rates_range quando o mercado está fechado)
import os
import json
import logging
...
# -------------------------
# safe_copy_rates (robusto com fallback para copy_rates_range)
# -------------------------
def safe_copy_rates(symbol: str, timeframe, count: int = 500) -> Optional[pd.DataFrame]:
    ...
    Tenta copy_rates_from_pos primeiro; se falhar ou retornar vazio, usa copy_rates_range com intervalo calculado.
    """
    try:
    ...
    try:
        ...
        for i in range(attempts):
            try:
                ...
                logger.debug(f"safe_copy_rates: copy_rates_from_pos error {e}")
        ...
        # Fallback para copy_rates_range se copy_rates_from_pos falhou ou retornou vazio
        if raw is None or len(raw) == 0:
            logger.debug(f"safe_copy_rates: fallback para copy_rates_range em {symbol}")
            # Mapear timeframe para minutos por barra
            tf_minutes_map = {
                mt5.TIMEFRAME_M1: 1,
                mt5.TIMEFRAME_M5: 5,
            ...
            minutes_per_bar = tf_minutes_map.get(timeframe, 15)  # Default M15
            utc_now = datetime.now(timezone.utc)
            now_time = int(utc_now.timestamp())
            from_dt = utc_now - timedelta(minutes=count * minutes_per_bar)
            from_time = int(from_dt.timestamp())
            try:
                raw = mt5.copy_rates_range(symbol, timeframe, from_time, now_time)
            except Exception as e:
                logger.debug(f"safe_copy_rates: copy_rates_range error {e}")
                raw = None

        if raw is None or len(raw) == 0:
            logger.debug(f"safe_copy_rates: sem dados para {symbol} após {attempts} tentativas e fallback")
            return None
...
# posição / risco
# -------------------------
def calculate_position_size(symbol: str, sl_price: float, risk_pct: float = None) -> Optional[float]:
    try:
        import config as cfg
        if risk_pct is None:
            try:
                risk_pct = float(getattr(cfg, 'RISK_PER_TRADE_PCT', 0.01))
            except Exception:
                risk_pct = 0.01
    except Exception:
        risk_pct = 0.01
