...
class LogAnalyzer:
    ...
    def list_log_files(self) -> List[Path]:
        files = list(self.log_dir.glob("analysis_log_*.txt"))
        files.extend(self.log_dir.glob("analysis_*.txt"))
        return sorted(files)
    ...
    def analyze_single_day(self, date_str: str = None) -> Dict:
        ...
        daily_file = self.log_dir / f"analysis_log_{date_str}.txt"
        bucket_files = sorted(self.log_dir.glob(f"analysis_{date_str}_*.txt"))
        files_to_parse = []
        if daily_file.exists():
            files_to_parse = [daily_file]
        elif bucket_files:
            files_to_parse = bucket_files
        else:
            return {"error": f"Arquivos não encontrados para {date_str}"}
        analyses = []
