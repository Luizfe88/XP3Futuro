...
def fast_backtest_core(
    ...
    # ✅ Initial Check
    if len(close) < 3:
        # include empty list when trivial
    ...
    trade_pnls: list[float] = []  # capture each closed trade P/L
                ...
                if position > 0:  # Long
                    ...
                    if high[i] >= mid_target:
                        ...
                        if qty_close > 0:
                            if asset_type == 1:
                                gross_profit = (
                                    (mid_target - entry_price) * point_value * qty_close
                                )
                                if fee_type == 1:
                                    cost = fee_val * qty_close
                                else:
                                    cost = (
                                        qty_close * mid_target
                    ...
                    if low[i] <= mid_target:
                        ...
                        if qty_close > 0:
                            if asset_type == 1:
                                gross_profit = (
                                    (entry_price - mid_target) * point_value * qty_close
                                )
                                if fee_type == 1:
                                    cost = fee_val * qty_close
                                else:
    ...
    for i in range(p, len(close)):
        ...
        if abs(position) > 0:
            ...
            if hit_tp or hit_sl:
                ...
                if hit_sl:
                    exit_price = (
                        stop_price * (1 - base_slippage)
                        if position > 0
                        else stop_price * (1 + base_slippage)
                    )
        ...
        equity_curve[i] = cash + (
            position * (price - entry_price) * pv if position != 0 else 0
        )
...
def backtest_params_on_df(symbol: str, params: dict, df: pd.DataFrame, ml_model=None):
    ...
    equity_arr, trades, wins, losses, sigs, counts, trade_pnls = fast_backtest_core(
        close,
        open_,
        high,
        low,
...
def run_optimization(
    ...
    The optimizer is expected to train on one window (e.g. 3 months) and then
    validate on a fully unseen out-of-sample window (e.g. the last month).
    Pass appropriate ``wfo_train_size``/``wfo_test_size`` values when
    calling this function.  A larger validation window improves the power of
    the permutation test by increasing the sample size.
    """
    import optuna
    ...
    def objective_wrapper(trial):
        ...
        try:
            if strategy_name == "MEAN_REVERSION":
                ...
                if "base_slippage" not in params:
                    params["base_slippage"] = 0.001
    ...
    if "sl_atr_multiplier" in best_params and "tp_ratio" in best_params:
        best_params["tp_mult"] = (
            best_params["sl_atr_multiplier"] * best_params["tp_ratio"]
        )
    ...
    if strategy_name == "MEAN_REVERSION":
        ...
        if "base_slippage" not in best_params:
            best_params["base_slippage"] = 0.001
        best_metrics = backtest_params_on_df(symbol, best_params, data, ml_model=None)

    # write trade history output before running permutation test (if available)
    try:
        import config

        cfg = getattr(config, "PERMUTATION_TEST", {})
        tr = best_metrics.get("trade_returns")
        if tr:
            path = cfg.get("trade_history_path", "ml_trade_history.json")
            with open(path, "w", encoding="utf-8") as f:
                json.dump([{"pnl": r} for r in tr], f)
    except Exception:
        pass

    # ------------------------------------------------------------------
    # 🔍 Permutation-test validation (experimental / institutional grade)
    # ------------------------------------------------------------------
    try:
        import config
        ...
        if cfg.get("enabled", False):
            ...
            metrics_to_check = cfg.get("metrics", ["profit_factor"])
            results_list = []
            for m in metrics_to_check:
                res = run_permutation_test(
                    trade_history_path=cfg.get(
            ...
            best_metrics["permutation_test"] = results_list
            agent = StatisticalValidatorAgent(
                p_value_threshold=cfg.get("p_value_threshold", 0.05)
            )
            passed = agent.evaluate(results_list)
            if not passed:
                best_metrics["permutation_failed"] = True
                logger.critical(
                    "🚨 KILL SWITCH ATIVADO - Estratégia reprovada no Monte Carlo (Possível Overfitting)"
                )
                try:
                    with open("kill_switch_active.txt", "w", encoding="utf-8") as f:
                        f.write(f"Permutation failed: {results_list}\n")
                except Exception:
                    pass

                # 📨Enviar alerta Telegram com gráfico anexado (se estiver configurado)
                try:
                    from utils import get_telegram_bot

                    bot = get_telegram_bot()
                    if bot and hasattr(bot, "send_photo"):
                        # pick first metric plot if available
                        plot_file = None
                        if results_list and isinstance(results_list, list):
                            plot_file = results_list[0].get("plot")
                        if plot_file and os.path.exists(plot_file):
                            with open(plot_file, "rb") as img:
                                bot.send_photo(
                                    chat_id=config.TELEGRAM_CHAT_ID,
                                    photo=img,
                                    caption="🚨 KILL SWITCH: estratégia reprovada no teste de permutação",
                                )
                except Exception:
                    pass
            else:
                best_metrics["permutation_failed"] = False
    except Exception as e:
        logger.warning(f"Erro ao executar permutation test: {e}")

    return best_params, best_metrics


def tournament_report(
    symbol: str, df: pd.DataFrame, n_trials: int = 50, timeout: int = 300
) -> dict:
    strategies_config = {
        "VOLATILITY_BREAKOUT": {
            "params": {
    ...
    for strat, conf in strategies_config.items():
        if strat not in ["VOLATILITY_BREAKOUT", "MEAN_REVERSION"]:
            continue
    ...
    try:
        ...
        with open(
            os.path.join("optimizer_output", "rejection_reasons.txt"),
            "a",
            encoding="utf-8",
        ) as f:
...
def objective(trial, symbol, df, ml_model=None):
    ...
    try:
        ...
        # Hard constraints introduced to drive the optimizer toward a large
        ...
        try:
            ...
            if penalty > 0:
                log_rejection(
                    ...
                    trial.number,
                    "PENALTY",
                    f"{' | '.join(reason)} | PF={pf:.2f}",
                )

            score = (
                (metrics["win_rate"] * 2.0) + (metrics["profit_factor"] * 1.2) - penalty
            )
            return -score
