...
def _standardize_params(params: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "ema_short": int(params.get("ema_fast", 8)),
        "ema_long": int(params.get("ema_slow", 21)),
        "adx_threshold": int(params.get("adx_threshold", 20)),
        "rsi_low": int(params.get("rsi_low", 30)),
        "rsi_high": int(params.get("rsi_high", 70)),
        "sl_atr_multiplier": float(params.get("sl_atr_mult", 2.5)),
        "tp_ratio": float(params.get("tp_atr_mult", 4.0)),
        "weight": 0.0,
    }
...
def generate_report(all_results: dict):
    ...
    # Pré-processa melhores resultados por símbolo e calcula pesos
    best_per_symbol = {}
    sharpe_sum = 0.0
    sharpe_list = {}
    for symbol, results in all_results.items():
        if results:
            ...
            sharpe = float(best['test_metrics'].get('sharpe', 0.0))
            sharpe_pos = max(0.0, sharpe)
            sharpe_list[symbol] = sharpe_pos
            sharpe_sum += sharpe_pos
            best_per_symbol[symbol] = best
        else:
            best_per_symbol[symbol] = None
    # Normaliza pesos; se todos zero, distribui igualmente
    if sharpe_sum > 0:
        weights = {s: (sh / sharpe_sum) for s, sh in sharpe_list.items()}
    else:
        count = max(1, len([s for s, b in best_per_symbol.items() if b]))
        weights = {s: (1.0 / count) for s, b in best_per_symbol.items() if b}
    ...
    with open(report_file_md, 'w', encoding='utf-8') as f:
        ...
        for symbol, results in all_results.items():
            ...
            if results is None or len(results) == 0:
                ...
                best = best_per_symbol.get(symbol)
                std_params = _standardize_params(best['params'])
                std_params["weight"] = float(weights.get(symbol, 0.0))
                f.write("### ✅ Melhores Parâmetros (Padronizados)\n\n")
                f.write("```python\n")
    ...
    json_file = os.path.join(OUTPUT_DIR, f"futures_params_{timestamp}.json")
    std_out = {}
    for symbol, best in best_per_symbol.items():
        ...
        std_params = _standardize_params(best['params'])
        std_params["weight"] = float(weights.get(symbol, 0.0))
        std_out[symbol] = std_params
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(std_out, f, indent=2)
