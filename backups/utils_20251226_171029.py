...
def calculate_signal_score(ind: dict) -> float:
    """
    ✅ VERSÃO COMPLETA: Inclui filtro de Momentum
    """
    ...
    momentum = ind.get("momentum", 0.0)  # ✅ NOVO!
    ...
    # ✅ MOMENTUM MÍNIMO (NOVO!)
    mom_min = params.get("mom_min", 0.0001)  # Padrão: 0.01% (0.0001)
    ...
    # ✅ FILTRO DE MOMENTUM (NOVO!)
    # Rejeita setups sem força/movimento
    ...
    if not ind.get("spread_ok", True):
        return {"valid": False, "reason": "Spread alto"}
    ...
    # RSI Score com limites do config
    ...
    # 🚀 MOMENTUM SCORE (NOVO!)
    # =========================
    # Bônus para momentum forte
    mom_abs = abs(momentum)
    
    if mom_abs >= mom_min * 5:
        # Momentum MUITO forte (5x o mínimo)
        mom_score = 15
        score_log["MOMENTUM"] = 15
    elif mom_abs >= mom_min * 3:
        # Momentum forte (3x o mínimo)
        mom_score = 10
        score_log["MOMENTUM"] = 10
    elif mom_abs >= mom_min:
        # Momentum ok (acima do mínimo)
        mom_score = 5
        score_log["MOMENTUM"] = 5
    else:
        # Não deveria chegar aqui (já filtrado)
        mom_score = 0
...
def get_current_risk_pct() -> float:
    ...
    if config.PROFIT_LOCK["enabled"] and config.PROFIT_LOCK["reduce_risk"]:
        ...
        if acc:
            # Busca equity_inicio_dia de um arquivo temporário
            try:
                if os.path.exists("daily_equity.txt"):
                    ...
                    if daily_pnl_pct >= config.PROFIT_LOCK["daily_target_pct"]:
                        risk *= 0.5
                        logger.debug(f"🔒 Risco reduzido para {risk*100:.2f}% (Meta diária atingida)")
            except:
                pass
...
def send_daily_performance_report():
    ...
    meta_atingida = daily_pnl_pct >= getattr(config.PROFIT_LOCK, "daily_target_pct", 0.02)
    ...
    msg = (
        ...
        f"🤖 <b>ML</b>: {total_trades} trades registrados | Epsilon: {getattr(ml_optimizer, 'epsilon', 0):.3f}\n\n"
        f"✅ Sistema operando normalmente"
    )
...
def calcular_lucro_realizado_txt():
    ...
    with open(filename, "r", encoding="utf-8") as f:
        ...
        for linha in linhas:
            # Ignora o cabeçalho e linhas de separação
            if "DATA/HORA" in linha or "---" in linha:
                continue
            
            # Só soma se NÃO for uma "Abertura de Posição"
            if "Abertura de Posição" not in linha:
                try:
                    # O P&L está entre "P&L:" e "R$"
                    # Exemplo: P&L: +1550.00 R$
                    parte_pnl = linha.split("P&L:")[1].split("R$")[0].strip()
                    total_pnl += float(parte_pnl)
                    contagem_trades += 1
